import axios from 'api/axios'

const { REACT_APP_BASE_URL: BASE_URL } = process.env

const getAll = (): Promise<any> => {
  return axios.get(`${BASE_URL}/company`)
}

const getById = (id: string): Promise<any> => {
  return axios.get(`${BASE_URL}/company/${id}`)
}

const getEligibleEmployeesByCompanyId = (companyId: string): Promise<any> => {
  return axios.get(`${BASE_URL}/company/${companyId}/eligible-employee`)
}

export const create = (data: any): Promise<any> => {
  return axios.post(`${BASE_URL}/company`, { data })
}

export const update = (id: string, data: any): Promise<any> => {
  return axios.put(`${BASE_URL}/company/${id}`, { data })
}

export const del = (id: string): Promise<any> => {
  return axios.delete(`${BASE_URL}/company/${id}`)
}

export default {
  getAll,
  getById,
  getEligibleEmployeesByCompanyId,
  create,
  update,
  delete: del
}
